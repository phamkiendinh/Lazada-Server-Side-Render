...
<body>
    ...
    <main>
        <div class="container-fluid d-flex flex-column justify-content-center align-items-center">
            <h1 class="d-none" id="notification">
                <?php
                if (isset($_POST['addProduct']) && isset($_POST['productName']) && isset($_POST['productPrice']) ) {
                    if ($_SESSION['type'] == "vendor") {
                        ...
                        $info = pathinfo($_FILES['productProfile']['name']);
                        $ext = $info['extension']; // get the extension of the file
                        $newname = $info['filename'] . '.' . $ext; 
                        
                        $target = './image/'.$newname;
                        move_uploaded_file( $_FILES['productProfile']['tmp_name'], $target);

                        $file = fopen("company/" . $name . ".csv", "a");
                        $newProduct = $_POST['productName'] . ',' . $_POST['productPrice'] . ',' . $_POST['productDescription'] . ',' . $target . "\n"; 
                        fwrite($file, $newProduct);
                        fclose($file);
                        echo "Product Added!";
                        echo '<script> document.getElementById("notification").className = "d-block text-success"; </script>';
                        }
                        
                } else {
                    echo '<script> document.getElementById("notification").className = "d-none"; </script>';
                }
                ?>
            </h1>
            <form class="form" action="" method="POST" enctype="multipart/form-data">
                <label for="productName" class="fw-bold gap-5">Product Name</label>
                <input type="text" name="productName" pattern="{10,20}" class="form-control">
                <label for="productPrice" class="fw-bold gap-5 mt-5">Product Price</label>
                <input type="text" name="productPrice" pattern="[0-9]+[^-]*" class="form-control">
                <label for="productDescription" class="fw-bold gap-5 mt-5">Product Description</label>
                <input type="text" name="productDescription" pattern="{1,500}" class="form-control">
                <label for="productProfile" class="fw-bold mt-5" id="">Choose Product Picture</label>
                <input type="file" name="productProfile" id="productProfile" class="form-control">
                <button type="submit" name="addProduct" class="btn btn-primary mt-5 form-control">Add Product</button>
            </form>
